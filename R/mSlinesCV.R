...
#' @param points logical; if plotting, FALSE to only plot lines and TRUE to add points on the mS score, i.e. result from [microSTASIS::stabilitas()].
#' @param sizeLine numeric; if plotting, size of the multiple lines.
#'
#' @return A line plotin the form of a [ggplot2::ggplot()] object with the values of stability for the multiple subsets and the original matrix of paired samples (points).
#' @export
#'
#' @examples
#' \donttest{
...
#' }
mSlinesCV <- function (pairedTime, CVklist,  k = 1L, points = TRUE, sizeLine = 0.5) {
  ...
  if (k == 1L) {
    object <- data.frame(x = stats::reorder(individuals, 
                                            sort(as.character(individuals))), y = unlist(CVklist), 
    ...
    removed_ind <- sapply(seq_along(CVsamples), function(sample) {
      stringr::str_c(individuals[which(samples %in% as.vector(stringr::str_split(CVsamples[sample], ", ", 
                                                                                 simplify = TRUE)))], 
                     collapse = ", ")
    })
    object <- data.frame(x = stats::reorder(removed_ind, sort(as.character(removed_ind))), 
                         y = unlist(CVklist), 
                         individual = sort(rep(individuals, length(CVklist[[1]]))))
  }
  p <- ggplot2::ggplot(object, ggplot2::aes(x = x, y = y, group = individual, colour = individual)) + 
    ggplot2::geom_line(size = sizeLine) + ggplot2::theme_bw() + ggplot2::labs(x = NULL, y = "Stability") + 
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1))
  if (points) {
      p + ggplot2::geom_point(data = if (k == 1L) {subset(object, x == individual)} else {
        subset(object, stringr::str_detect(x, individual))
