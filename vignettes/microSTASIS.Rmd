...
samples <- 1:dim(clr)[1]
metadata <- data.frame(samples, stringr::str_split(ID_common_time, "_", simplify = T))
colnames(metadata)[2:4] <- c("ID", "common", "time_point")
both_tse <- TreeSummarizedExperiment(assays = list(counts = t(clr)), colData = metadata)
altExp(both_tse) <- SummarizedExperiment(assays = SimpleList(data = t(clr)), colData = metadata)
...
TSE_altExp_times <- pairedTimes(data = both_tse, sequential = TRUE, assay = "data",
                                 alternativeExp = "unnamed1",
                                 ID = "ID", timePoints = "time_point")
```
...
Note that the downstream functions work equally once we have the `pairedTimes()`output.

```{r "iterativeClusteringTSE"}
mS_TSE <- iterativeClustering(pairedTimes = TSE_times, parallel = TRUE, common = "_")
```
